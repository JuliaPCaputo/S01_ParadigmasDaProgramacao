...
falta_concluir(A, P) :- prerequisito(D, P), \+ concluiu(A, P).

falta_prerequisito(A, D) :- prerequisito(D, P), \+ concluiu(A, P).

aluno_apto(A, D) :- disciplina(D, _, Area), Area \= fundamental, \+ concluiu(A, D), \+ falta_prerequisito(A, D).

% query 1: aluno_apto(joao, D).
% query 2: aluno_apto(maria, inteligencia_artificial).