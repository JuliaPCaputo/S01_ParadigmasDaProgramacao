...
disciplina_raiz(D) :- disciplina(D, _, _), \+ prerequisito(_, D).

disciplina_folha(D) :- disciplina(D, _, _), \+ prerequisito(D, _).

% query: disciplina_raiz(D), \+ disciplina(D, _, aplicacoes)