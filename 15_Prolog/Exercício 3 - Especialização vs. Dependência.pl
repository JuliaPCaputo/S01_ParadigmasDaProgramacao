...
especialista_comp(Aluno) :- findall(D, (concluiu(Aluno, D), disciplina(D, _, computacao)), DisciplinasComp), length(DisciplinasComp, N), N >= 2.

deficiencia_mat(Aluno) :- \+ (concluiu(Aluno, D), disciplina(D, _, matematica)).

% query: aluno(A), especialista_comp(A), deficiencia_mat(A)